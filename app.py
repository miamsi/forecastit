...
st.markdown("Version 1: Stealth Data Fetch (Bypasses Streamlit IP Blocking)")
...
# --- 1. DATA INGESTION ENGINE (Version 1 - Stealth Session Fix) ---
@st.cache_data(ttl=3600)
def fetch_data(symbol, years):
    try:
        # Create a session with a browser-like User-Agent to avoid IP blocking
        ...
        if data.empty:
            st.sidebar.warning(f"Yahoo returned empty data for {symbol}. IP might be throttled.")
            return None
            
        # Ensure we have the required columns and handle potential MultiIndex
...
if df is not None:
    ...
    st.error("Could not load data. Yahoo Finance is currently rejecting the connection from this server.")
    st.info("💡 **Pro-Tip:** Try clicking the 'Manage App' button at the bottom right, then 'Reboot App' to refresh the server's IP address.")
